...
def export(config, ALL_sections: list, z_planes, pitch, chord, mesh, mesh_DUST, connectivity_DUST, coordinates_DUST, DUST_export = True):
    ...
    ###################     Normal mesh export     ###################
    meshio.write(f'{config["FILE"]["output_dir"]}/{config["FILE"]["name_tag"]}.vtk', mesh, file_format='vtk')

    ## Generate array for ALL_sections (NODE_ID + coordinates)
    ALL_sections_array = np.concatenate(ALL_sections, axis=0)

    with open(f'{config["FILE"]["output_dir"]}/{config["FILE"]["name_tag"]}.pts', 'w') as file:
    ...
    np.savetxt(f'{config["FILE"]["output_dir"]}/{config["FILE"]["name_tag"]}_pitch_chord.txt', 
               np.concatenate([z_planes[:, None]/float(config["PANELS"]["z_max"]), chord[:, None], pitch[:,None]], axis=1), header='r_R' + '\t' + 'chord (mm)' + '\t' +'pitch(deg)', delimiter='\t')
    
    if DUST_export:

        ###################       DUST export     ###################
        DUST_dir = f'{config["FILE"]["output_dir"]}/DUST_output'
