...
def export_mesh(config, case_name, ALL_sections: list, z_planes, pitch, chord, mesh, mesh_DUST, connectivity_DUST, coordinates_DUST, DUST_export = True):
...
def export_blade(config, case_name_sec, blade):
    ...
    ## If case folder does not exist, create it
    case_dir = f'{config["FILE"]["output_dir"]}/{case_name_sec}'
    if os.path.isdir(case_dir) is False:
        os.mkdir(case_dir)

    ##  Export the step file    ##
    cq.exporters.export(blade, f'{case_dir}/{case_name_sec}.step')


def export_blade_mesh(config, case_name_sec, ALL_sections: list, mesh, mesh_DUST, connectivity_DUST, coordinates_DUST, DUST_export = True):
    ...
    if DUST_export:
        ...
        ## vtk export for visualisation
        meshio.write(f'{DUST_dir}/{case_name_sec}_DUST.vtk', mesh_DUST, file_format='vtk')
